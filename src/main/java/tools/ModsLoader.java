        ...
        if (filesArray.length > 0) {
            for (File jar : filesArray) {
                if (jar.getName().endsWith(".jar")) {
                    ...
                    try {
                        existingJarMap.put(jar.getName(), new JarFile(jarPath));
                    } catch (IOException e) {
...
//						e.printStackTrace();
    ...
    private static void preparing(Path path) {
        ...
        if (!Files.isDirectory(path)) {
            throw new RuntimeException("Income 'modsDir' must be a DIRECTORY, not a file!");
        }
    }
...
//		    	e.printStackTrace();
    ...
    private void cashing(JarFile jarFile, JarEntry jarEntry) {
        byte[] classData = null;
        try {
            classData = loadClassData(jarFile, jarEntry);
        } catch (Exception b) {
        ...
        if (classData != null) {
            ...
            if (clazz != null) {
                this.cacheClass.put(jarCutted, clazz);
            } else {
                Out.Print(ModsLoader.class, LEVEL.ERROR, "Нельзя положить в карту класс, равный null: " + jarCutted + " (" + clazz + ")");
            }
        } else {
            Out.Print(ModsLoader.class, LEVEL.ERROR, "При чтении класса мода " + jarFile + " произошла ошибка!");
        }
    }
        ...
        for (String pathName : this.cacheClass.keySet()) {
            if (pathName.endsWith("Mod")) {
                ...
                try {
                    ...
                    try {
                        System.err.println("parent on!");
                        loadClass(pathName);
                    } catch (ClassNotFoundException e1) {
                        try {
                            System.err.println("super on!");
                            super.loadClass(pathName);
                        } catch (ClassNotFoundException e2) {
                            System.err.println("Exception e2 fail!");
                        }
                    }
